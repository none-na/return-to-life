...
local sprites = {
    gateFX = Sprite.load("TPGateway", "Graphics/tpGateway", 1, 22, 24),
    mask = Sprite.load("Graphics/gatewayMask", 1, 20.5, 20.5),
    spark = Sprite.load("Graphics/tpSparks", 8, 6, 4),
    ----------------------------------
...
tpGateway:addCallback("step", function(self)
    ...
    if data.activity > -1 then
        ...
        elseif data.activity == 3 then --create exp and shit
...
orb:addCallback("step", function(self)
    ...
    if boundTo and boundTo:isValid() then
        ...
        self.x = boundTo.x + ((orbTravelRadius*math.sin(data.i/(orbTravelRadius*5))))
