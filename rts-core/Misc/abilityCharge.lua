...
local hudElement = Sprite.load("Graphics/skillCharge", 2, 9, 8)
    ...
    for _, ability in pairs(abilities) do
        ...
        if a[enableVars[ability]] == 1 then
            ...
            if actor:getAlarm(ability) > a[stopVars[ability]] then
                --print("Skill alarm is above its stop value!")
                -- remove a charge
                if a[chargeVars[ability]] > 0 then
                    local var = actor:getAlarm(ability) --* (1-a[cdrVars[ability]])
                    ...
                    --print("Deducting a charge, setting internal timer")
                end
            else
                ...
                if a[chargeVars[ability]] < 1 then
                    ...
                    --print("Out of charges, setting vanilla alarm to internal alarm")
                end
            end
            ...
            if a[alarmVars[ability]] > -1 then --Check if our internal timer should be counting...
                ...
                if (a[alarmVars[ability]] % ((a[cooldownVars[ability]] * (1-a[cdrVars[ability]])) + 1) == 0) then
                    if a[chargeVars[ability]] < a[maxChargeVars[ability]] then
                        --print("Adding a charge")
                        if a[chargeVars[ability]] < 1 then
                            --print("There are more than zero charges, setting vanilla alarm to zero")
                ...
                if a[chargeVars[ability]] > 0 and a[chargeVars[ability]] < a[maxChargeVars[ability]] then
                    --print("Player doesn't have enough charges, resetting timer")
            ...
            if actor:getAlarm(ability) == -1 and a[chargeVars[ability]] >= a[maxChargeVars[ability]] then
                --print("No need to keep counting, setting internal timer to zero")
...
callback.register("onPlayerHUDDraw", function(player, x, y)
    ...
    for _, ability in pairs(abilities) do
        if p[enableVars[ability]] == 1 then
            ...
            -- debug shit
            --graphics.print(p[maxChargeVars[ability]], x + drawCoordinates[ability].x, y + drawCoordinates[ability].y - (graphics.textHeight("0", NewDamageFont) * 4), NewDamageFont, graphics.ALIGN_MIDDLE, graphics.ALIGN_CENTER)
            --graphics.print(p[cooldownVars[ability]] * (1-p[cdrVars[ability]]), x + drawCoordinates[ability].x, y + drawCoordinates[ability].y - (graphics.textHeight("0", NewDamageFont) * 6), NewDamageFont, graphics.ALIGN_MIDDLE, graphics.ALIGN_CENTER)
            --graphics.print(player:getAlarm(ability), x + drawCoordinates[ability].x, y + drawCoordinates[ability].y - (graphics.textHeight("0", NewDamageFont) * 8), NewDamageFont, graphics.ALIGN_MIDDLE, graphics.ALIGN_CENTER)
            --graphics.print(p[alarmVars[ability]], x + drawCoordinates[ability].x, y + drawCoordinates[ability].y - (graphics.textHeight("0", NewDamageFont) * 10), NewDamageFont, graphics.ALIGN_MIDDLE, graphics.ALIGN_CENTER)
            --graphics.print(p[alarmVars[ability]] % (p[cooldownVars[ability]] * (1-p[cdrVars[ability]])), x + drawCoordinates[ability].x, y + drawCoordinates[ability].y - (graphics.textHeight("0", NewDamageFont) * 12), NewDamageFont, graphics.ALIGN_MIDDLE, graphics.ALIGN_CENTER)
            --graphics.print(p[stopVars[ability]], x + drawCoordinates[ability].x, y + drawCoordinates[ability].y - (graphics.textHeight("0", NewDamageFont) * 14), NewDamageFont, graphics.ALIGN_MIDDLE, graphics.ALIGN_CENTER)
        end
    end
end)
